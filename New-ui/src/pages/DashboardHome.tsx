...
export default function DashboardHome() {
  ...
  const { data: resumes = [], isLoading: resumesLoading } = useQuery<Resume[]>({
    ...
    queryFn: async () => {
      ...
      return Array.isArray(res.data) ? res.data : res.data?.resumes || []
    },
    staleTime: 2 * 60 * 1000,
  })

  const { data: scans = [], isLoading: scansLoading } = useQuery<ATSScan[]>({
    queryKey: ['dashboard-scans'],
    queryFn: async () => {
      const res = await api.get('/ats/history')
      return Array.isArray(res.data) ? res.data : res.data?.scans || []
    },
    staleTime: 2 * 60 * 1000,
  })

  const statsLoading = resumesLoading || scansLoading
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {/* Plan banner — renders immediately */}
      {user?.plan === 'free' && (
      ...
      {/* Stats — skeleton only for these 4 cards */}
      ...
      {/* Quick actions — renders immediately, no loading gate */}
      <div className="mb-8">
