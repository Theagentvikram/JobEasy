...
function Section({
  ...
  return (
    <div className="border border-slate-200 dark:border-dark-border-subtle rounded-xl overflow-hidden">
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between px-4 py-3 bg-white dark:bg-dark-card hover:bg-slate-50 dark:hover:bg-dark-hover/50 cursor-pointer transition-colors"
      >
        <span className="text-sm font-semibold text-slate-900 dark:text-slate-100 flex items-center gap-2">
          {title}
          {badge !== undefined && (
            <span className="text-xs font-medium text-slate-400 dark:text-slate-500 bg-slate-100 dark:bg-dark-elevated px-1.5 py-0.5 rounded-full">
              {badge}
            </span>
          )}
        </span>
      ...
      {open && <div className="p-4 border-t border-slate-100 dark:border-dark-border-subtle space-y-3 bg-white dark:bg-dark-card">{children}</div>}
    </div>
  )
}
...
export default function ResumeEditor() {
  ...
  if (loading) return <div className="flex items-center justify-center h-screen bg-white dark:bg-dark-bg"><Spinner /></div>
  if (!resume) return null

  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 dark:bg-dark-bg">
      {/* Left editor panel */}
      <div className="w-[420px] flex-shrink-0 flex flex-col border-r border-slate-100 dark:border-dark-border bg-white dark:bg-dark-surface">
        {/* Toolbar */}
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          ...
          <Section title="Experience" badge={resume.experience.length}>
            {resume.experience.map((exp) => (
              <div key={exp.id} className="border border-slate-100 dark:border-dark-border-subtle rounded-lg p-3 space-y-2 bg-white dark:bg-dark-card">
                <div className="flex items-center gap-2">
          ...
          <Section title="Education" badge={resume.education.length}>
            {resume.education.map((edu) => (
              <div key={edu.id} className="border border-slate-100 dark:border-dark-border-subtle rounded-lg p-3 space-y-2 bg-white dark:bg-dark-card">
          ...
          <Section title="Projects" badge={resume.projects.length} defaultOpen={false}>
            {resume.projects.map((proj) => (
              <div key={proj.id} className="border border-slate-100 dark:border-dark-border-subtle rounded-lg p-3 space-y-2 bg-white dark:bg-dark-card">
                <div className="flex gap-2">
                  <div className="flex-1 grid grid-cols-2 gap-2">
                    <Input
      ...
      {/* Right preview panel — bg-slate-200/dark:bg-dark-elevated so white resume "paper" pops */}
      <div className="flex-1 overflow-y-auto bg-slate-200 dark:bg-dark-elevated p-8">
