...
const STATUS_CONFIG: Record<JobStatus, { label: string; color: string; bg: string; badge: 'default' | 'brand' | 'warning' | 'success' | 'danger' | 'accent' }> = {
  waiting_referral: { label: 'Waiting referral', color: 'text-slate-600', bg: 'bg-slate-50', badge: 'default' },
  referral_received: { label: 'Referral received', color: 'text-blue-700', bg: 'bg-blue-50', badge: 'brand' },
  apply_today: { label: 'Apply today', color: 'text-amber-700', bg: 'bg-amber-50', badge: 'warning' },
  applied: { label: 'Applied', color: 'text-brand-700', bg: 'bg-brand-50', badge: 'brand' },
  interview: { label: 'Interview', color: 'text-violet-700', bg: 'bg-violet-50', badge: 'accent' },
  offer: { label: 'Offer', color: 'text-green-700', bg: 'bg-green-50', badge: 'success' },
  rejected: { label: 'Rejected', color: 'text-red-700', bg: 'bg-red-50', badge: 'danger' },
  withdrawn: { label: 'Withdrawn', color: 'text-slate-500', bg: 'bg-slate-50', badge: 'default' },
  closed: { label: 'Closed', color: 'text-slate-400', bg: 'bg-slate-50', badge: 'default' },
}
            ...
            <>
              ...
              <div className="absolute right-0 top-6 z-20 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-lg overflow-hidden w-40">
                ...
                {Object.entries(STATUS_CONFIG).map(([s, c]) => (
                  <button
                    key={s}
                    onClick={() => { onStatusChange(job.id, s as JobStatus); setMenuOpen(false) }}
                    className={cn(
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-slate-100 dark:border-slate-800">
            {['Role', 'Company', 'Status', 'Source', 'Date', 'Priority', ''].map((h) => (
                  ...
                  <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                    {job.link && (
                      <a href={job.link} target="_blank" rel="noopener noreferrer"
                        className="p-1 text-slate-400 hover:text-brand-700 dark:hover:text-brand-400 cursor-pointer">
...
function AddJobModal({ open, onClose, onAdd }: {
  ...
  const [form, setForm] = useState({
    ...
    link: '', jobDescription: '', status: 'waiting_referral' as JobStatus,
  ...
  const submit = async () => {
    ...
    try {
      ...
      setForm({
        title: '', company: '', location: '', source: '', jobType: 'Full-time',
        link: '', jobDescription: '', status: 'waiting_referral',
        priority: 0, waitingPeriod: 7, notes: '',
      })
    } catch {
...
export default function JobTracker() {
  ...
  useEffect(() => {
    api.get('/referral/jobs').then((res) => {
    ...
    <div className="p-6 max-w-full">
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          title={filterStatus === 'all' ? 'No jobs tracked yet' : `No jobs with status "${STATUS_CONFIG[filterStatus as JobStatus]?.label}"`}
