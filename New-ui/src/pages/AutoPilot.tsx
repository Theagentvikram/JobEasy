...
export default function AutoPilotPage() {
  ...
  async function handleSearch() {
    ...
    if (resumeText.trim().length < 50) {
      toast.warning('Paste your resume text (at least 50 characters)')
      return
    }
    ...
    <div className="flex flex-col gap-6 max-w-6xl mx-auto">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          Enter keywords → get 50 jobs with curated, tailored resumes and match scores. Ready to apply in one click.
        </p>
      </div>
            ...
            <Card className="p-6 flex flex-col gap-5">
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Paste your resume text here. The AI will use this to score and tailor a version for each job…"
                  rows={10}
                  className="w-full px-3 py-2 border border-slate-200 dark:border-slate-600 rounded-lg text-sm bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-50 focus:outline-none focus:ring-2 focus:ring-brand-700 resize-y placeholder:text-slate-400"
                />
                <p className="text-xs text-slate-400 mt-1">
                  Tip: paste plain text from your resume editor or copy from a PDF
                </p>
              </div>
              ...
              <Button
                ...
                size="lg"
                className="w-full"
                disabled={keywords.length === 0 || resumeText.trim().length < 50}
              >
                <Zap size={16} />
